...
%% ========================================================================
    ...
    function onEditTracker()
        ...
        editDesc = uieditfield(glId, 'text', 'Value', curDesc, ...
            'Tooltip', 'Optional description shown in the tracker list.');
        ...
        switch upper(trackerType)
            case 'GNN'
                ...
                editGate = uieditfield(glAlg, 'numeric', 'Value', curGate, ...
                    ...
                    'Tooltip', 'Mahalanobis distance squared. Higher = looser association. Typical: 25–50.');
                uilabel(glAlg, 'Text', 'Confirmation threshold:');
                editConfirm = uieditfield(glAlg, 'numeric', 'Value', curConfirm, ...
                    'RoundFractionalValues', 'on', ...
                    'Tooltip', 'Track score threshold to confirm. Higher = stricter. Typical: 20.');
                uilabel(glAlg, 'Text', 'Deletion threshold:');
                editDelete = uieditfield(glAlg, 'numeric', 'Value', curDelete, ...
                    'RoundFractionalValues', 'on', ...
                    'Tooltip', 'Track score for deletion. Typically negative. Typical: −5.');
            ...
            case 'JPDA'
                ...
                editNumTrJ = uieditfield(glAlg, 'numeric', 'Value', curNumTracksJ, ...
                    ...
                    'Tooltip', 'Max concurrent JPDA tracks. Typical: 100–500.');
                uilabel(glAlg, 'Text', 'Confirm probability:');
                editJpdaConf = uieditfield(glAlg, 'numeric', 'Value', curJpdaConf, ...
                    'Limits', [0 1], ...
                    'Tooltip', 'Existence probability to confirm a track. Must exceed Delete prob. Typical: 0.95.');
                uilabel(glAlg, 'Text', 'Delete probability:');
                editJpdaDel = uieditfield(glAlg, 'numeric', 'Value', curJpdaDel, ...
                    'Limits', [0 1], ...
                    'Tooltip', 'Existence probability below which a track is deleted. Typical: 0.05.');
                saveEnabled = true;
            case 'TOMHT'
                uilabel(glAlg, 'Text', 'Gate (Mahalanobis):');
                editGate = uieditfield(glAlg, 'numeric', 'Value', curGate, ...
                    'Limits', [0 Inf], 'LowerLimitInclusive', false, ...
                    'Tooltip', 'Mahalanobis distance squared. Higher = looser. Typical: 25–50.');
                uilabel(glAlg, 'Text', 'Confirmation threshold:');
                editConfirm = uieditfield(glAlg, 'numeric', 'Value', curConfirm, ...
                    'RoundFractionalValues', 'on', ...
                    'Tooltip', 'Track score threshold to confirm. Higher = stricter. Typical: 20.');
                ...
                editFarMht = uieditfield(glAlg, 'numeric', 'Value', curFarMht, ...
                    'Limits', [0 1], 'LowerLimitInclusive', false, ...
                    'Tooltip', 'TOMHT false alarm rate per scan. Typical: 1e-6.');
                uilabel(glAlg, 'Text', 'Volume:');
                editVolume = uieditfield(glAlg, 'numeric', 'Value', curVolume, ...
                    'Limits', [0 Inf], 'LowerLimitInclusive', false, ...
                    'Tooltip', 'Measurement cell volume in m³. Typical: 1e8 to 1e10.');
                ...
                editMaxBr = uieditfield(glAlg, 'numeric', 'Value', curMaxBranch, ...
                    'Limits', [1 Inf], 'RoundFractionalValues', 'on', ...
                    'Tooltip', 'Hypothesis branches before pruning. Higher = slower but more accurate. Typical: 100–200.');
                ...
                editMult1 = uieditfield(glAlg, 'numeric', 'Value', curMult1, ...
                    'Limits', [0 Inf], ...
                    'Tooltip', 'Confirm gate multiplier (smallest). AssignmentThreshold(1). Typical: 1.0.');
                uilabel(glAlg, 'Text', 'Threshold mult (history):');
                editMult2 = uieditfield(glAlg, 'numeric', 'Value', curMult2, ...
                    'Limits', [0 Inf], ...
                    'Tooltip', 'History gate multiplier. AssignmentThreshold(2). Typical: 1.5–2.0.');
                uilabel(glAlg, 'Text', 'Threshold mult (deletion):');
                editMult3 = uieditfield(glAlg, 'numeric', 'Value', curMult3, ...
                    'Limits', [0 Inf], ...
                    'Tooltip', 'Deletion gate multiplier (largest). AssignmentThreshold(3). Typical: 2.5–3.0.');
                saveEnabled = true;
            otherwise
                uilabel(glAlg, ...
        ...
        function onSave()
            ...
            % v3.5 §Step2.5 — cross-field sensibility checks. Caught here
            % (after name validation, before overwrite confirm) so the
            % user sees the warning before deciding whether to clobber
            % an existing file. "Save anyway" path is allowed because
            % an experimenter may want intentionally weird values; the
            % warning just makes it conscious.
            switch upper(trackerType)
                case 'GNN'
                    if editConfirm.Value <= editDelete.Value
                        ok = confirmCrossFieldWarning(editorFig, sprintf( ...
                            ['Confirmation threshold (%g) is not greater than ' ...
                             'Deletion threshold (%g). Tracks may delete as ' ...
                             'fast as they confirm.\n\nSave anyway?'], ...
                            editConfirm.Value, editDelete.Value));
                        if ~ok; return; end
                    end
                case 'JPDA'
                    if editJpdaConf.Value <= editJpdaDel.Value
                        ok = confirmCrossFieldWarning(editorFig, sprintf( ...
                            ['Confirm probability (%.3f) is not greater than ' ...
                             'Delete probability (%.3f). Tracks may never ' ...
                             'reach confirmed state.\n\nSave anyway?'], ...
                            editJpdaConf.Value, editJpdaDel.Value));
                        if ~ok; return; end
                    end
                case 'TOMHT'
                    if editConfirm.Value <= editDelete.Value
                    ...
                    m1 = editMult1.Value; m2 = editMult2.Value; m3 = editMult3.Value;
                    if ~(m1 <= m2 && m2 <= m3)
                        ok = confirmCrossFieldWarning(editorFig, sprintf( ...
                            ['Threshold multipliers should satisfy ' ...
                             'Confirm ≤ History ≤ Deletion.\n\n' ...
                             'Current: Confirm=%g, History=%g, Deletion=%g.\n\n' ...
                             'Save anyway?'], m1, m2, m3));
                        if ~ok; return; end
                    end
            end
...
function ok = confirmCrossFieldWarning(parentFig, msg)
%confirmCrossFieldWarning  v3.5 §Step2.5 — wrap uiconfirm for the
%   tracker editor's cross-field sensibility warnings. Returns true
%   if the user chose "Save anyway", false on Cancel. Default and
%   close-X both map to Cancel.
    choice = uiconfirm(parentFig, msg, 'Cross-field check', ...
        'Options', {'Save anyway', 'Cancel'}, ...
        'DefaultOption', 'Cancel', ...
        'CancelOption',  'Cancel', ...
        'Icon', 'warning');
    ok = strcmp(choice, 'Save anyway');
end

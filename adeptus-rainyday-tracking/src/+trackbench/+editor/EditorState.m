...
%    relaunching pathEditor.
    ...
    properties
        ...
        % ── Curve options (M4) ───────────────────────────────────────
        %   M4.3.1/3.2: straight is the M3 default and must remain the
        %   default on fresh editor launch. "curved" enables centripetal
        %   Catmull-Rom interpolation through the Nx5 control waypoints
        %   for rendering and (in §3.4) export. Alpha/tension is fixed at
        %   0.5 for now; §3.3 adds the Uniform/Centripetal/Chordal dropdown.
        curveMode        (1,1) string  = "straight"  % "straight" | "curved" (M4.3.2)
        curveTensionAlpha (1,1) double = 0.5         % 0=uniform, 0.5=centripetal, 1=chordal (M4.3.3 wires UI)
        curveDensityPerSeg (1,1) double = 50         % dense points per control-waypoint segment (M4.3.1)
        ...
        % ── M4 UI handles ────────────────────────────────────────────
        curveModeBtn       = gobjects(1)     % M4.3.2 (Straight / Smooth toggle)
        curveTensionDD     = gobjects(1)     % M4.3.3 (Uniform / Centripetal / Chordal) — UI wired in §3.3
    ...
    methods
        ...
        function snap = snapshot(obj)
            ...
            % M4: curve mode + tension participate in undo/redo so a
            % straight-↔-curved flip behaves like every other edit.
            snap.curveMode          = obj.curveMode;
            snap.curveTensionAlpha  = obj.curveTensionAlpha;
        end
        ...
        function restore(obj, snap)
            ...
            % M4: restore curve mode + tension (fields added mid-release,
            % so tolerate older snapshots that lack them — treat as the
            % M3 defaults).
            if isfield(snap, 'curveMode') && ~isempty(snap.curveMode)
                obj.curveMode = snap.curveMode;
            else
                obj.curveMode = "straight";
            end
            if isfield(snap, 'curveTensionAlpha') && ~isempty(snap.curveTensionAlpha)
                obj.curveTensionAlpha = snap.curveTensionAlpha;
            else
                obj.curveTensionAlpha = 0.5;
            end
            obj.recomputeTimes();
            obj.isDirty = true;
        end

...
autoTuneOnlyGNN   = false;      % keep it focused; you can expand later
% ------------------------------------
...
% ================= SCENARIO SELECT ==================
scenarioMode = "2D";   % "2D" uses helperCreateScenario
...
%% GNN track-management thresholds
% IMPORTANT:
%   Your trackerGNN class does NOT have a 'ClutterDensity' property.
%   To make Pd/FAR/Volume/Beta *relevant* (and eliminate warnings), we run
%   GNN in *Score* track logic (not History).
%
% Score logic uses scalar thresholds:
%   - ConfirmationThreshold: positive scalar (track score must exceed)
%   - DeletionThreshold: negative scalar (track score drop to delete)
if enableDegradation
    confirmGNN = 30;   % more conservative in RAINY
    deleteGNN  = -10;  % less aggressive deletion in RAINY
else
    confirmGNN = 20;
    deleteGNN  = -7;
end
    ...
    if meetsTolerance(bestParts, tol)
        ...
        for it = 1:maxIters
            ...
            tracker = trackerGNN( ...
                ...
                'ConfirmationThreshold', confirmGNN, ...
                'DeletionThreshold', deleteGNN, ...
                'DetectionProbability', tuneIter.pd, ...
                'FalseAlarmRate', farGNN_it, ...
                'Volume', vol, ...
                'Beta', tuneIter.beta);
...
tracker = trackerGNN( ...
    ...
    'AssignmentThreshold', gateGNN_GNNONLY, ...
	    ...
	    'ConfirmationThreshold', confirmGNN, ...
	    'DeletionThreshold', deleteGNN, ...
	    'DetectionProbability', pd_GNNONLY, ...
	    'FalseAlarmRate', farGNN_GNNONLY, ...
	    'Volume', vol, ...
	    'Beta', beta_GNNONLY);
